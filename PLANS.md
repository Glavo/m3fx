...
- Demo interaction visual coverage checks hover and pressed-state snapshots for buttons, sidebar destinations, and toggle icon buttons, verifies button and toggle icon button ripple-release intermediate fade frames, and covers focus feedback for text fields plus no-motion state changes.
- Demo page-level visual coverage now shares geometry assertions across full-page, dark expressive, and right-to-left passes, verifying visible text and Material control bounds against the scene and scroll viewports while allowing expected partial scroll-edge content.
...
- Expand animated intermediate visual coverage to more stateful components such as navigation rows, split buttons, switches, and picker popups.
