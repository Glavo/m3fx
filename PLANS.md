...
- Slot, container, split-button, and popup-backed controls route accessibility focus to the currently focused child when focus is inside their indexed or popup-hosted content, actively notify `FOCUS_NODE` when child focus changes, and fall back to the first reachable focus target for default focus requests.
- Indexed and slot-based controls support default accessibility reveal behavior: parameterless `SHOW_ITEM` focuses the selected, current, or first reachable indexed child as appropriate for each control, direct child and descendant `Node` parameters are accepted when contained by an indexed item, carousels scroll the selected item while restoring control focus, virtualized list views restore the active row focus target, FAB menus expand before revealing actions, and snackbar hosts refresh hosted snackbar skins before routing focus to current or queued snackbar actions.
