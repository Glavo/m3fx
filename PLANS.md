...
- Indexed and slot-based controls support default accessibility reveal behavior: parameterless `SHOW_ITEM` focuses the first reachable indexed child, direct child and descendant `Node` parameters are accepted when contained by an indexed item, FAB menus expand before revealing actions, and snackbar hosts refresh hosted snackbar skins before routing focus to current or queued snackbar actions.
