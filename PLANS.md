...
- Pickers: calendar date picker with selected date, displayed month, first-day-of-week, optional adjacent-month days, and inclusive min/max date range support; time picker with 12-hour/24-hour display, minute steps, keyboard adjustment, and inclusive min/max time range support; date and time picker fields with editable text, popup picker selection, formatter-based parsing, range validation, and shared popup motion.
...
- Continue filling component gaps such as date-range picker composition, richer validation presentation patterns, and higher-level form helpers.
