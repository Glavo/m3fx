...
- Core popup, overlay, snackbar, tooltip, sheet, scrim, search, and drawer-group completion tests prefer state-driven JavaFX condition waits over hard-coded elapsed-time waits; fixed delays remain only where the test intentionally samples an intermediate animation frame, hover delay, or ongoing indeterminate motion.
...
- Popup menu accessibility tests cover selected-item default focus for standalone menus, menu buttons, and submenus without changing explicit first/last keyboard-open behavior. Mixed popup focus tests cover rich tooltip actions opened from menu items while the parent menu remains active, dialog-pane focus routing through nested menu-button and picker popups, and shared external popup focus routing through indexed containers, surface content subtrees, and slot-based app bars. Dialog pane accessibility tests cover current content/action preservation for parameterless focus and reveal actions, including picker dialog preset and picker-content targets plus nested picker popups exposed through ordinary content containers. Picker field accessibility tests cover open-button focus exposure and preservation of focused preset actions for direct `REQUEST_FOCUS` and parameterless `SHOW_ITEM` actions.

## Next Goals

- Continue auditing the remaining timing assumptions in tests and visual snapshots, especially hover-delay, intermediate-frame, and indeterminate-motion samples, so completion checks describe semantic state instead of elapsed time.
