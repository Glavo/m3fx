...
- List visual coverage opens the real Lists demo page, verifies that the data-driven list is backed by `VirtualFlow`, asserts that only a bounded number of reusable cells are attached for 240 rows, scrolls to a far row synchronously, and writes a dedicated virtualized-list snapshot for visual review.
...
- Core popup, overlay, snackbar, tooltip, sheet, scrim, search, progress, submenu, and drawer-group completion plus intermediate-frame tests prefer state-driven JavaFX condition waits over hard-coded elapsed-time waits where the controls expose observable state. Demo visual tests also support stable-condition waits that require final popup, sheet, snackbar, and FAB menu states to remain true for consecutive JavaFX pulses before settled snapshots are captured.
- Demo animation visual tests capture progress, loading indicator, split-button popup, picker-field popup, nested submenu, hover/pressed interaction feedback, switch selection, navigation selection, drawer group disclosure, sheet visibility, snackbar, and FAB-menu frames after real rendered pixel changes are detected, so those snapshots no longer rely on fixed early popup, generic interaction, or broad midpoint transition delays. Ripple release coverage combines rendered release-frame pixel changes with semantic opacity fade checks, and rich tooltip lifetime coverage relies on popup interaction ownership plus the control's built-in pointer-transfer grace instead of a test-owned hide delay. Core and demo visual test utilities no longer expose fixed-delay FX sampling helpers; remaining time values are component motion configuration, direct timeline frame probes, or state-driven pulse waits.
...
- Popup menu accessibility tests cover selected-item default focus for standalone menus, menu buttons, and submenus without changing explicit first/last keyboard-open behavior. Mixed popup focus tests cover rich tooltip actions opened from menu items while the parent menu remains active, dialog-pane focus routing through nested menu-button and picker popups, shared external popup focus routing through indexed containers, surface content subtrees, and slot-based app bars, plus explicit reveal of closed nested menu targets from those composite owners. Dialog pane accessibility tests cover current content/action preservation for parameterless focus and reveal actions, including picker dialog preset and picker-content targets plus nested picker popups exposed through ordinary content containers. Picker field accessibility tests cover open-button focus exposure and preservation of focused preset actions for direct `REQUEST_FOCUS` and parameterless `SHOW_ITEM` actions.

## Next Goals

- Continue reducing the remaining intentional timing samples in interaction visual tests, especially real dialog and popup setup snapshots, by replacing them with semantic or rendered-pixel conditions where a stable target state exists.
