...
- Popup menu accessibility tests cover selected-item default focus for standalone menus, menu buttons, and submenus without changing explicit first/last keyboard-open behavior. Dialog pane accessibility tests cover current content/action preservation for parameterless focus and reveal actions, including picker dialog preset and picker-content targets. Picker field accessibility tests cover open-button focus exposure and preservation of focused preset actions for direct `REQUEST_FOCUS` and parameterless `SHOW_ITEM` actions.
