...
/// A Material Design 3 settings row that performs an action.
///
/// `M3SettingItem` is suitable for a settings entry that opens another screen, presents a dialog, or performs an
/// immediate operation. It does not imply navigation, persistence, or a selected value. Install an action handler
/// with [#setOnAction(javafx.event.EventHandler)] to define the operation performed when the row is activated.
///
...
public final class M3SettingItem extends M3SettingItemBase {
    /// The concrete style class assigned to action setting rows.
